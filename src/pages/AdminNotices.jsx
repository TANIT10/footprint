...
function AdminNotices({
  ...
  const [changingFeaturedId, setChangingFeaturedId] = useState(null)
    ...
    const loadNotices = async () => {
      ...
      try {
        ...
        if (response.status === 401) {
          throw new Error('로그인 시간이 만료됐어요.')
        }

        if (response.status === 403) {
          throw new Error('관리자 권한을 확인할 수 없어요.')
        }
        ...
        const noticePage = await response.json()

        setNotices(
          Array.isArray(noticePage.notices)
            ? noticePage.notices
            : []
        )
      } catch (error) {
  ...
  const handleSetFeatured = async (
    ...
    if (!token) {
      window.alert('로그인 정보를 찾을 수 없어요.')
      return
    }

    const confirmed = window.confirm(
      `"${notice.title}" 공지를 대표 공지로 설정할까요?`
    )
    ...
    try {
      setChangingFeaturedId(notice.id)
      ...
      if (!response.ok) {
        throw new Error('대표 공지를 설정하지 못했어요.')
      }

      setNotices((previousNotices) =>
        previousNotices.map((previousNotice) => ({
          ...previousNotice,
          featured:
            previousNotice.id === notice.id,
        }))
      )

      window.alert('대표 공지로 설정했어요.')
    } catch (error) {
      console.error('대표 공지 설정 실패:', error)
  ...
  const handleClearFeatured = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('대표 공지를 해제하지 못했어요.')
      }
      ...
      window.alert('대표 공지를 해제했어요.')
    } catch (error) {
      console.error('대표 공지 해제 실패:', error)
            ...
            {notices.map((notice) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={
                    changingFeaturedId !== null
                  }
                >
                  <Star aria-hidden="true" />

                  <span>
                    {changingFeaturedId === notice.id
                      ? '변경 중...'
                      : notice.featured
                        ? '대표 공지 해제'
                        : '대표로 설정'}
